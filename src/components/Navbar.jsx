...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* TOP CURTAIN */}
      <div
        className={`fixed inset-x-0 top-0 h-1/2 bg-[#E8DCC4] z-30 md:hidden transition-transform duration-700 ease-[cubic-bezier(0.77,0,0.175,1)] ${isOpen ? "translate-y-0" : "-translate-y-full"
          }`}
      />

      {/* BOTTOM CURTAIN */}
      <div
        className={`fixed inset-x-0 bottom-0 h-1/2 bg-[#E8DCC4] z-30 md:hidden transition-transform duration-700 ease-[cubic-bezier(0.77,0,0.175,1)] ${isOpen ? "translate-y-0" : "translate-y-full"
          }`}
      />
      ...
      <div
        ...
        className={`fixed inset-0 z-40 flex flex-col items-center justify-center md:hidden transition-opacity duration-500 delay-100 ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
      >
